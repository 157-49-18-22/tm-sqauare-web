...
const Quote = () => {
  ...
  const submit = e => {
    e.preventDefault();
    setSent(true);
    setForm({ name:'', email:'', service:'Automotive Safety (TPMS)', message:'' });
    setTimeout(() => setSent(false), 4000);
  };

  return (
    <section style={{ background:'#0d0d0d', padding:'120px 0', borderTop:'1px solid rgba(255,255,255,0.04)', position:'relative', overflow:'hidden' }}>
      {/* Accent blobs */}
          ...
          <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:20 }}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
              {[
                ...
                <div key={f.name}>
                  <label style={{ display:'block', fontSize:'0.65rem', fontWeight:700, letterSpacing:'0.15em', textTransform:'uppercase', color:'#555', marginBottom:8 }}>{f.label}</label>
                  <input
                    type={f.type} name={f.name} value={form[f.name]}
                    onChange={change} required placeholder={f.placeholder}
            ...
            <div>
              <label style={{ display:'block', fontSize:'0.65rem', fontWeight:700, letterSpacing:'0.15em', textTransform:'uppercase', color:'#555', marginBottom:8 }}>Service Required</label>
              <select name="service" value={form.service} onChange={change} className="form-control" style={{ cursor:'pointer' }}>
                <option>Automotive Safety (TPMS)</option>
                <option>Office Logistics (Paper)</option>
                <option>RFID / FASTag Services</option>
                <option>Other Inquiry</option>
              </select>
            </div>

            <div>
              <label style={{ display:'block', fontSize:'0.65rem', fontWeight:700, letterSpacing:'0.15em', textTransform:'uppercase', color:'#555', marginBottom:8 }}>Message</label>
              <textarea
            ...
            <button
              type="submit"
              style={{ background:'var(--accent)', color:'#fff', border:'none', borderRadius:8, padding:'16px 32px', fontFamily:"'Bebas Neue',sans-serif", fontSize:'1.2rem', letterSpacing:'0.15em', cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:12, transition:'all 0.2s', marginTop:8 }}
